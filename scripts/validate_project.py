...
REQUIRED = [
    ...
    "docs/08_COMMANDS.md", "docs/09_SOURCES.md", "state/setup_status.json",
...
def validate(root: Path) -> dict:
    ...
    for field in ("default_inspection_ids", "default_generation_priority_ids",
                  "optional_scene_style_ids", "optional_world_reference_ids"):
        for rid in project.get(field, []):
            if rid not in ids:
                errors.append(f"Unknown {field} reference: {rid}")
            elif rid.startswith("R-"):
                errors.append(f"Review-only reference in default generation path: {rid}")
    ...
    return {"ok": not errors, "root": str(root), "master_images": masters,
            "review_only_images": reviews, "agents_bytes": size,
...
def main() -> int:
    ...
    if args.json:
        ...
        for key in ("master_images", "review_only_images", "agents_bytes"):
            if key in report:
